...
then
    ...
    echo "What is your node name prefix exmaple:app-xx.ocp.example.com the name prefix is "app-" dont forget the "-" after node name prefix"
    read=prefix

    echo "What is your domain prefix and suffix exmaple:app-xx.ocp.example.com the full domain is ".ocp.example.com" dont forget the dot before cluster name prefix ".ocp""
    read=domain

    echo "How many worker nodes do you have in your cluster please note the script patches all worker nodes exmaple: 67"
    read machine_count

    echo Your have enterd you have $machine_count worker nodes

    echo "How many IP addresses would you like to patch *SELECT NUMBER* - (1 IP) (2 IP's) (3 IP's) \
    ...
    then
        ...
        echo "A report will be generated indicating all of the updated hostsubnet's for each node \
        file path is ./hostsub_report.txt"

        oc get hostsubnet | awk '{print $1, $5, $6, $7}' >> ./hostsub_report.txt
        echo "Report genration finished"
...
then
    ...
    echo "How many IP addresses would you like to add to a project  *SELECT NUMBER* - (1 IP) (2 IP's) (3 IP's) \
    ...
    then
            ...
            echo "A report will be generated indicating all of the updated egressIP's for each project\
            file path is ./prject_egressIPs_report.txt"

            oc get netnamespace | awk '{print $1, $3, $4, $5}' >> ./prject_egressIPs_report.txt
            echo "Report genration finished"
